...
def function():
    ...
    model_path = os.path.join(SCRIPT_DIR,'resnet152.pth')

    model_process_begin = datetime.datetime.now()
    model = resnet152(pretrained=False)
    model.load_state_dict(torch.load('resnet152.pth'))  # Ensure the model is loaded to the right device
    model.to(device)
    model.eval()
    ...
    input_batch = preprocess(input_image).unsqueeze(0).to(device)   # create a mini-batch as expected by the model
